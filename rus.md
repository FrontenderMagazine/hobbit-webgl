...
фильме про хоббита («Хоббит: Пустошь Смауга»). Создание комплексного мобильного
мультимедиа-эксперимента для Chrome было действительно вдохновляющей и сложной
задачей.

Этот сайт оптимизирован под мобильный Chrome на новых устройствах семейства
Nexus, где доступен WebGL и Web Audio. Однако большая часть сайта доступна и
...
поддерживать больше устройств с простыми эффектами или, как мы, ограничить 
поддерживаемые устройства теми, которые способны отобразить более реалистичный 
трёхмерный мир. Для «Путешествия через Средиземье» мы сосредоточились на 
устройствах серии Nexus и пяти популярных смартфонах на базе Android.

В этом эксперименте мы использовали [three.js][1], так как уже делали с его
помощью несколько проектов использующих WebGL. Сначала был создан первоначальный
вариант [игры Trollshaw][2], который должен был хорошо работать на планшете Nexus 10.
После первоначального тестирования на устройстве, у нас появился список пунктов 
оптимизации, которые были не менее актуальны и для слабых ноутбуков:
...
* Рендерить WebGL вполовину размера и масштабировать `canvas` используя CSS.

После того, как игра была оптимизирована, мы смогли получить стабильные 30 
кадров в секунду. Теперь наша цель была в том, чтобы улучшить внешний вид 
без негативного влияния на частоту кадров. Мы попробовали разные подходы: 
некоторые оказали действительно большое влияние на производительность, а 
некоторые — нет.
...
обнаружили, что можно значительно увеличить сложность моделей без
ощутимого ухудшения производительности. Модели троллей, которые мы
используем в этой игре, имеют около 5 тысяч граней, а сама сцена — примерно 
40 тысяч, и этого вполне достаточно.
...
это как раз то, что оказывает на мобильных устройствах большое влияние на 
производительность.
...
Выбор материалов может значительно повлиять на производительность на
мобильном устройстве и к этому нужно подходить с умом. Использование
...
При разработке игр для мобильных устройств, GPU — не всегда самое большая проблема.
Много времени требует CPU, в особенности на физику и скелетные анимации. 
Один из трюков, который иногда помогает, в зависимости от симуляции — 
это делать «дорогие» перерасчеты каждый второй кадр. Вы также можете
...
которые мы повторно используем. При повторном использовании объектов 
...
<iframe src="examples/maze.htm" frameborder="0" width="440" height="420">
</iframe>
...
компьютеру подключена мышь и клавиатура, а на мобильных устройствах пользователи
взаимодействуют с помощью жестов, растягивания, ориентации устройства в
пространстве и т.д.

## Тактильное взаимодействие на мобильных устройствах

Добавить поддержку тач-событий не так уж сложно. Есть [много материалов][14] по
этой теме. Но есть пара мелочей, которые могут все усложнить.
...
интересующие вас касания. Чтобы отличать тап от свайпа мы используем задержку и
проверяем, произошло ли движение касания (свайп) или нет (тогда это тап). Чтобы
...
*Пример обнаружения свайпа для игры Trollshaw. Намжите и проведите курсором мышки, 
либо сделайте свайп, если у вас у вас тачскрин, по серой области.*
...
Нашим общим впечатлением от этого проекта: WebGL на мобильных
...
шейдеры и размер холста WebGL — это участки, требующие основной оптимизации на
мобильных устройствах. Однако быстродействие достигается за счет множества 
составляющих, так что оптимизируйте все, что сможете.
...
экрана. В некоторых случаях нам приходилось перемещать 3D-камеру ближе, чтобы на
самом деле видеть, что происходит.
